...
function CurrentProjectID() {
	...
	return "{F31DB578-461D-4551-B52B-112914F68329}";
}
...
class cAdvancedSecurity {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($valid) {
						...
						$this->setSessionUserID($rs->fields('StudentID')); // Load User ID
						if (is_null($rs->fields('userlevel_id'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('userlevel_id'))); // Load User Level
						}
						$this->SetupUserLevel();
	...
	function GetUserIDByUserName($UserName) {
		...
		if (strval($UserName) <> "") {
			...
			$sSql = $UserTable->GetSQL($sFilter, '');
			if (($RsUser = $UserTableConn->Execute($sSql)) && !$RsUser->EOF) {
				$UserID = $RsUser->fields('StudentID');
				$RsUser->Close();
				return $UserID;
			}
		}
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($RsUser = $UserTableConn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('StudentID'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
